...
import { calculateTravelOptions, TravelOption, TravelSession, cancelTravelSession } from '../../services/travelEngine';
...
export function TravelDrawer({
  ...
  const handleConfirmTravel = () => {
    ...
    const session: TravelSession = {
      ...
      originName: currentRegion.name,
      originCountry: currentRegion.country,
    ...
    <View style={styles.overlayContainer}>
      <View style={styles.drawerCard}>
        <View style={styles.header}>
          <Text style={styles.title}>✈️ BORANG PERJALANAN GEOPOLITIK</Text>
        ...
        <View style={styles.routeBox}>
          <View style={styles.routeNode}>
            <Text style={styles.routeLabel}>ASAL</Text>
        ...
        <Text style={styles.secLabel}>PILIH MOD PENGANGKUTAN:</Text>
            ...
            return (
              ...
              >
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  <Text style={[styles.optName, isSelected && styles.optNameSelected]}>
                    {opt.name}
                  </Text>
                  <Text style={styles.optTime}>⏱ Tempoh Transit: {opt.durationMinutes} minit</Text>
                </View>
        ...
        <View style={styles.summaryBox}>
          <View style={styles.summaryRow}>
            <Text style={styles.sumLabel}>Tiket Tambang:</Text>
            <Text style={styles.sumVal}>RM {selectedMode.ticketCost}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.sumLabel}>Cukai Pelancongan ({targetRegion.country}):</Text>
          ...
          <View style={[styles.summaryRow, { marginTop: 6, paddingTop: 6, borderTopWidth: 1, borderColor: '#334155' }]}>
            <Text style={styles.totalLabel}>JUMLAH KOS PERJALANAN:</Text>
        ...
        <TouchableOpacity style={styles.btnConfirm} onPress={handleConfirmTravel}>
          <Text style={styles.btnConfirmTxt}>MULAKAN PERJALANAN NOW ➔</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export function TravelProgressBar({
  ...
  onCancelTravel: (newReturningSession: TravelSession) => void;
}) {
    ...
    <View style={barStyles.container}>
      <View style={barStyles.headerRow}>
        <Text style={barStyles.title}>
          {iconMap[session.mode] || '✈️'}{' '}
          {session.isReturning ? '↩️ BERPATAH BALIK KE ' : 'DALAM PERJALANAN KE '}
          <Text style={{ color: '#FBBF24' }}>{session.targetName.toUpperCase()}</Text>
        </Text>
        <Text style={barStyles.timerTxt}>{timeLeft > 0 ? formattedTime : 'TIBA!'}</Text>
      </View>

      <View style={barStyles.track}>
        <View style={[barStyles.fill, { width: `${progressPercent}%`, backgroundColor: session.isReturning ? '#EF4444' : '#10B981' }]} />
      </View>

      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
        <Text style={barStyles.subTxt}>
          {session.isReturning ? 'Sedang berpatah balik ke pangkalan asal...' : 'Sila tunggu kenderaan sampai...'}
        </Text>

        {/* Butang Batal & Patah Balik (Hanya paparkan jika belum berpatah balik) */}
        {!session.isReturning && (
          <TouchableOpacity
            style={barStyles.btnCancel}
            onPress={() => {
              const returningSession = cancelTravelSession(session);
              onCancelTravel(returningSession);
            }}
          >
            <Text style={barStyles.btnCancelTxt}>↩️ BATAL & PATAH BALIK</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  drawerCard: { width: 420, maxHeight: '85%', backgroundColor: '#0F172A', borderWidth: 1.2, borderColor: '#D97706', borderRadius: 8, padding: 16 },
  ...
  sumVal: { color: '#F8FAFC', fontSize: 10, fontWeight: 'bold' },
...
const barStyles = StyleSheet.create({
  container: { position: 'absolute', bottom: 20, left: '15%', right: '15%', backgroundColor: 'rgba(15, 23, 42, 0.96)', borderWidth: 1.5, borderColor: '#D97706', borderRadius: 8, padding: 14, zIndex: 90 },
  ...
  fill: { height: '100%' },
  subTxt: { color: '#64748B', fontSize: 9 },
  btnCancel: { backgroundColor: '#7F1D1D', borderWidth: 1, borderColor: '#EF4444', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 4 },
  btnCancelTxt: { color: '#FCA5A5', fontSize: 9, fontWeight: 'bold' },
});